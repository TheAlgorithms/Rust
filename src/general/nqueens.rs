...
fn main()
{
	...
	{
		println!("Running algorithm with 8 as a default. Specify an alternative Chess board size for N-Queens as a command line argument.\n");
		board_width = 8;
	}
...
pub fn nqueens(board_width : i64) -> Result<Vec<i64>, &'static str>
{
	...
	{
		conflict = false;
		for review_index in 0 .. current_row	//Column review
		{
			...
			if board_rows[current_row] == board_rows[review_index] || (left >= 0 && left == board_rows[current_row]) || (right < board_width as i64 && right == board_rows[current_row])
			{
				conflict = true;
				break;
			}
	...
	fn check_board(board : &Vec<i64>) -> bool
	{
		for current_row in 0 .. board.len()
		{
			for review_index in 0 .. current_row	//Column review
			{
				...
				if board[current_row] == board[review_index] || (left >= 0 && left == board[current_row]) || (right < board.len() as i64 && right == board[current_row])
				{
					return false;
				}
			}
