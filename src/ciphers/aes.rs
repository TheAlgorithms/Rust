...
fn key_expansion(init_key: &[Byte], num_rounds: usize) -> Vec<Byte> {
    ...
    let key = init_key.chunks(AES_WORD_SIZE)
        .map(|bytes| bytes_to_word(bytes))
        .collect::<Vec<Word>>();
...
fn add_round_key(data: &mut [Byte], round_key: &[Byte]) {
    ...
    data.into_iter().zip(round_key.repeat(num_blocks)).for_each(|(s, k)| *s ^= k);
}
...
fn padding<T: Clone + Default>(data: &[T], block_size: usize) -> Vec<T> {
    if data.len() % block_size == 0 {
        ...
        padded.append(&mut vec![T::default(); num_blocks * block_size - data.len()]);
        return padded;
    }
}
...
fn mix_column(block: &mut [Byte], mode: AesMode) {
    ...
    for col in 0 .. 4 {
        let col_word = block.iter()
            .zip(0 .. AES_BLOCK_SIZE).filter_map(|(&x, i)| {
                if i % 4 == col { Some(x) } else { None }
            }).collect::<Vec<u8>>();
        for row in 0 .. 4 {
            block[row * 4 + col] = (
                GF_MUL_TABLE[mix_col_matrix[row][0]][col_word[0] as usize] as Word ^ 
                GF_MUL_TABLE[mix_col_matrix[row][1]][col_word[1] as usize] as Word ^ 
                GF_MUL_TABLE[mix_col_matrix[row][2]][col_word[2] as usize] as Word ^ 
                GF_MUL_TABLE[mix_col_matrix[row][3]][col_word[3] as usize] as Word
            ) as Byte;
        }
    }
}
...
fn bytes_to_word(bytes: &[Byte]) -> Word {
    ...
    (bytes[0] as Word) <<  0 | (bytes[1] as Word) << 8 | 
    (bytes[2] as Word) << 16 | (bytes[3] as Word) << 24
}

fn word_to_bytes(word: Word) -> AesWord {
    let mut bytes = [0; AES_WORD_SIZE];
    bytes[0] = ((word & 0x000000ff) >>  0) as Byte;
    bytes[1] = ((word & 0x0000ff00) >>  8) as Byte;
    bytes[2] = ((word & 0x00ff0000) >> 16) as Byte;
    bytes[3] = ((word & 0xff000000) >> 24) as Byte;
    return bytes;
}
...
mod tests {
    ...
    fn test_aes_192() {
        ...
        let cipher: [u8; 16] = [0xdd, 0xa9, 0x7c, 0xa4, 0x86, 0x4c, 0xdf, 0xe0, 0x6e, 0xaf, 0x70, 0xa0, 0xec, 0x0d, 0x71, 0x91];
    ...
    fn test_aes_256() {
        ...
        let cipher: [u8; 16] = [0x8e, 0xa2, 0xb7, 0xca, 0x51, 0x67, 0x45, 0xbf, 0xea, 0xfc, 0x49, 0x90, 0x4b, 0x49, 0x60, 0x89];
    ...
    fn test_str() {
        ...
        let encrypted = aes_encrypt(plain, AesKey::AesKey128(key));
        let decrypted = aes_decrypt(&encrypted, AesKey::AesKey128(key));
        assert_eq!(str, String::from_utf8(decrypted).unwrap().trim_end_matches("\0"));
    }
