...
/// let rotation = 3;
/// let encrypted = caesar_cipher(text, rotation);
...
mod tests {
    ...
    fn rotation_at_alphabet_end() {
        assert_eq!(caesar_cipher("Hello, World!", 25), "Gdkkn, Vnqkc!");
    }
    ...
    fn encryption_and_decryption() {
        ...
        let rotation = 5;
        let encrypted_text = caesar_cipher(original_text, rotation);

        assert_eq!(caesar_cipher(&encrypted_text, 26 - rotation), original_text);
    }
    ...
    fn input_non_alphabetic_characters() {
        assert_eq!(caesar_cipher("12345!@#$%", 3), "12345!@#$%");
    }
    ...
    #[test]
    fn input_mixed_case() {
        assert_eq!(caesar_cipher("HeLlO WoRlD", 7), "OlSsV DvYsK");
    }
