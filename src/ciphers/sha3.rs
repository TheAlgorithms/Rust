...
mod tests {
    ...
    fn sha3_filling() {
        ...
        state_fill(&mut state, &arr);

        iterate!(x, y, z => {
            assert_eq!(state[x][y][z], true);
        })
    }
    ...
    fn sha3_dumping() {
        ...
        assert_eq!(arr, state_dump(&state));
    }

    #[test]
    fn sha3_h2b() {
        let bytes = [0xFFu8, 0x13u8];

        let bits = h2b(&bytes, bytes.len() * U8BITS);
    ...
    fn sha3_b2h() {
        ...
        assert_eq!(bytes, vec![0xFFu8, 0x13u8]);
    }

    #[test]
    fn sha3_224_short0() {
        let digest = sha3_224(&[0; 0]);

        let expected: [u8; 224 / U8BITS] = [
    ...
    #[test]
    fn sha3_256_short0() {
        let digest = sha3_256(&[0; 0]);

        let expected: [u8; 256 / U8BITS] = [
    ...
    #[test]
    fn sha3_384_short0() {
        let digest = sha3_384(&[0; 0]);

        let expected: [u8; 384 / U8BITS] = [
    ...
    #[test]
    fn sha3_512_short0() {
        let digest = sha3_512(&[0; 0]);

        let expected: [u8; 512 / U8BITS] = [
